...
class StreamService {
    ...
    async initialize(database) {
        try {
            ...
            log('FFmpeg binary path: %s', ffmpeg_1.default.path);
            log('FFmpeg version: %s', ffmpeg_1.default.version);
    ...
    async startStream(config) {
        ...
        try {
            ...
            log('🎬 Starting %s stream %s: %s → %s', platform, streamId, config.inputUrl.replace(/\/\/.*@/, '//[CREDENTIALS]@'), config.platform?.type === 'youtube' ? 'YouTube' :
    ...
    buildPlatformUrl(platform) {
        log('Building platform URL - type: %s, hasStreamKey: %s, hasServerUrl: %s', platform.type, !!platform.streamKey, !!platform.serverUrl);
        switch (platform.type) {
            ...
            default:
                ...
                throw new shared_1.StreamError(`Unsupported platform type: ${platform.type}`);
        }
    }
    buildYouTubeUrl(streamKey) {
        if (!streamKey) {
            throw new shared_1.StreamError('YouTube stream key is required');
        }
        return `rtmp://a.rtmp.youtube.com/live2/${streamKey}`;
    }
    buildTwitchUrl(streamKey) {
        if (!streamKey) {
            throw new shared_1.StreamError('Twitch stream key is required');
        }
        return `rtmp://live.twitch.tv/app/${streamKey}`;
    }
    spawnFFmpegProcess(options) {
        const args = this.buildFFmpegArgs(options);
        log('🚀 Starting FFmpeg: %s → %s', options.input.replace(/\/\/.*@/, '//[CREDENTIALS]@'), options.output?.substring(0, 50) + '...' || 'stdout');
        const process = (0, child_process_1.spawn)(ffmpeg_1.default.path, args, {
    ...
    setupStreamMonitoring(stream) {
        ...
        const monitorInterval = setInterval(() => {
            ...
            if (timeSinceLastProgress > 30000) {
                log('Stream %s ⚠️  No progress for %dms. Status: %s, FPS: %s, Size: %s', stream.id, timeSinceLastProgress, stream.status, stream.stats.fps, stream.stats.size);
                ...
                log('Stream %s ✅ Streaming: %s FPS, %s, %s speed', stream.id, stream.stats.fps, stream.stats.size, stream.stats.speed);
                lastLogTime = currentTime;
            }
        }, 30000); // Check every 30 seconds
        // Store the interval so we can clean it up
        stream.monitorInterval = monitorInterval;
    }
    ...
    async testRtspConnection(rtspUrl) {
        return new Promise((resolve) => {
            ...
            const process = (0, child_process_1.spawn)(ffmpeg_1.default.path, args, { stdio: ['pipe', 'pipe', 'pipe'] });
            let stdout = '';
            let stderr = '';
            process.stdout?.on('data', (data) => {
    ...
    getFFmpegInfo() {
        return {
            path: ffmpeg_1.default.path,
            version: ffmpeg_1.default.version
        };
    }
    // Health check
    async healthCheck() {
        try {
            ...
            log('Health check using FFmpeg path: %s', ffmpeg_1.default.path);
            const testProcess = (0, child_process_1.spawn)(ffmpeg_1.default.path, ['-version'], { stdio: 'pipe' });
            return new Promise((resolve) => {
